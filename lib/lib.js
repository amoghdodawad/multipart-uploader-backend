                ...
                bb.on('file', ( name, file, info ) => {
                    ...
                    file.on('end', () => {
                        ...
                        if(!fs.existsSync(`${this.#chunksDirectory}/${req.body.uploadId}`)){
                            fs.mkdirSync(`${this.#chunksDirectory}/${req.body.uploadId}`, { recursive: true })
                        }          
                        fs.writeFile(`${this.#chunksDirectory}/${req.body.uploadId}/${req.body.chunkIndex}.${this.#files[req.body.uploadId].extension}`, finalBuffer, (err) => {
    ...
    async completeUpload(uploadId){
        return new Promise(async ( resolve, reject ) => {
            ...
            if (totalChunks === uploadedChunks) {
                try {
                    if(!fs.existsSync(`${this.#videoDirectory}/${uploadId}`)){
                        fs.mkdirSync(`${this.#videoDirectory}/${uploadId}`, { recursive: true });
                    }
                    
                    for(let i = 0; i < totalChunks; ++i){
                        await appendToFile(`${this.#videoDirectory}/${uploadId}/${uploadId}.${this.#files[uploadId].extension}`, 
                        await readFromFile(`${this.#chunksDirectory}/${uploadId}/${i}.${this.#files[uploadId].extension}`));
                    }
